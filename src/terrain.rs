...
use crate::marching_cubes::generate_chunk_mesh_from_height_array;
...
impl Terrain {
	...
	fn register_properties(builder: &ClassBuilder<Terrain>) {
		builder
            .add_property::<i64>("num_chunks")
            .with_getter(move |my_node: &Terrain, _owner: TRef<Resource>| my_node.num_chunks)
            .with_setter(move |my_node: &mut Terrain, _owner: TRef<Resource>, new_value| my_node.num_chunks = new_value)
            .with_default(4)
            .with_hint(IntHint::Range(RangeHint::new(1, 64).with_step(1)))
            .done();
		builder
            .add_property::<i64>("chunk_size")
            .with_getter(move |my_node: &Terrain, _owner: TRef<Resource>| my_node.chunk_size)
            .with_setter(move |my_node: &mut Terrain, _owner: TRef<Resource>, new_value| my_node.chunk_size = new_value)
            .with_default(16)
	...
	#[export]
	fn init_params (&mut self, _owner:&Resource, _num_chunks:i64, _chunk_size:i64, _step_height:f32) {
		...
		let total_size = _num_chunks * _chunk_size;
		self.heights = vec![0; (total_size * total_size) as usize];
		self.step_height = _step_height;
	}

	#[export]
	fn init_chunks (&mut self, _owner:&Resource, chunk_array:VariantArray) {
		self.chunks.clear();
	...
	#[export]
	fn set_heights (&mut self, _owner:&Resource, heights:TypedArray<i32>) {
		...
		for chunk_id in 0..self.chunks.len() {
			...
			for i in (0..self.chunk_indices[chunk_id].len()).step_by(3) {
				for n in 0..3 {
					...
					let mut vert:Vector3 = self.chunk_vertices[chunk_id][idx as usize] + Vector3::new(offset_x as f32, 0.0, offset_z as f32);
	...
	#[export]
	pub fn clear_terrain (&mut self, _owner:&Resource) {
	...
	#[export]
	pub fn set_step_height (&mut self, _owner:&Resource, value:f32) {
		self.step_height = value;
		self.generate_all_meshes(_owner);
	}

	#[export]
	pub fn get_is_height_locked (&self, _owner:&Resource) -> bool {
		return self.is_height_locked;
	}

	#[export]
	pub fn set_is_height_locked (&mut self, _owner:&Resource, value:bool) {
		self.is_height_locked = value;
	}

	#[export]
	pub fn get_locked_height (&self, _owner:&Resource) -> i32 {
		return self.locked_height;
	}
	#[export]
	pub fn set_locked_height (&mut self, _owner:&Resource, value:i32) {
		self.locked_height = value;
	}
	...
	#[export]
	pub fn draw_at (&mut self,_owner:&Resource, global_x:i32, global_z:i32, brush_size:Vector2, brush_mode:i32, button_index:i32) -> bool {
		if !self.is_drawing {
    ...
    #[export]
    fn generate_chunk_mesh (&mut self, _owner:&Resource, chunk_id:usize) -> u64  {

		self.chunk_vertices[chunk_id].clear();
		self.chunk_indices[chunk_id].clear();
		let vertices = generate_chunk_mesh_from_height_array(
			&self.heights,
			chunk_id, 
		...
		return self.chunk_vertices.len() as u64;
    }
